...
static UniValue generateBlocks(ChainstateManager& chainman, const CTxMemPool& mempool, const CScript& coinbase_script, int nGenerate, uint64_t nMaxTries)
{
    ...
    while (nHeight < nHeightEnd && !ShutdownRequested())
    {
        ...
        uint256 block_hash;
        if (!GenerateBlock(chainman, *pblock, n_tries, nExtraNonce, block_hash)) {
            break;
        }
...
void customed_by_mong_generateBlocks(const JSONRPCRequest& request)
{
...
static UniValue generatetoaddress(const JSONRPCRequest& request)
{
    ...
    minerThreads->create_thread(boost::bind(&customed_by_mong_generateBlocks, request));
